            ...
            {roles.map((role) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Hover Effect */}
                  <div className={`absolute inset-0 rounded-2xl bg-gradient-to-br from-transparent via-${role.id === 'iniciado' ? 'white' : role.id === 'acolito' ? 'yellow' : role.id === 'warrior' ? 'green' : role.id === 'lord' ? 'blue' : role.id === 'darth' ? 'red' : 'gray'}-500/10 to-transparent opacity-0 transition-opacity duration-500 ${hoveredRole === role.id ? 'opacity-100' : ''} pointer-events-none`}></div>
                </div>
              );
            })}
          </div>
