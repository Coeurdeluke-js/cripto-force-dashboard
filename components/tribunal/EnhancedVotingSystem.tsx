...
import { ThumbsUp, ThumbsDown, Clock, CheckCircle, XCircle, Edit3 } from 'lucide-react';
...
export default function EnhancedVotingSystem({ proposal, onVote, onEdit }: EnhancedVotingSystemProps) {
  ...
  const canVote = (userData?.user_level || 0) >= 5; // Solo Darth y Maestros pueden votar
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#444] rounded-lg p-6">
      ...
      <div className="mb-6">
        ...
        {/* Indicador de carga */}
        {loading && (
          <div className="mb-4 p-4 bg-[#2a2a2a] border border-[#444] rounded-lg text-center">
          ...
          <div className={`p-4 rounded-lg border ${
            ...
            <div className="flex items-center justify-center space-x-2">
              ...
              <span className="font-medium">
                {isApproved ? 'Propuesta Aprobada por Unanimidad' : 'Propuesta Rechazada por Unanimidad'}
              </span>
            </div>
      ...
      <div className="space-y-3">
        ...
        {!loading && maestrosSistema.length > 0 ? (
          maestrosSistema.map((maestro) => {
            ...
            return (
              <div key={maestro.id} className={`flex items-center justify-between p-3 rounded-lg border border-[#444] ${
                hasVoted ? 'bg-[#1a1a1a]' : 'bg-[#2a2a2a]'
              }`}>
                <div className="flex items-center space-x-3">
                  {/* Icono de estado principal - Clock siempre visible */}
                  <div className={`p-2 rounded-full ${
                    hasVoted ? 'bg-[#333]' : 'bg-yellow-900/20'
                  }`}>
                    <Clock 
                      size={16} 
                  ...
                  <div>
                    <div className="font-medium text-white">
                      {maestro.nickname}
                      {isCurrentUser && (
                        <span className="ml-2 text-xs bg-[#FFD447] text-[#1a1a1a] px-2 py-1 rounded-full">
                          Tú
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center space-x-2">
                  {/* Pulgar hacia arriba - Verde si aprobado, gris si no - INTERACTIVO */}
                  <button
                    onClick={() => handleVote('approve')}
                    disabled={votingStatus === 'voting' || hasVoted || !canVote}
                    className={`p-1.5 rounded-full transition-all duration-200 ${
                      voteStatus.status === 'approved' 
                        ? 'bg-green-900/20 text-green-400' 
                        : 'bg-[#333] text-gray-400 hover:bg-[#444] hover:text-gray-300'
                    } ${!hasVoted && canVote ? 'cursor-pointer' : 'cursor-default'}`}
                    title={!hasVoted && canVote ? 'Votar Aprobar' : 'Ya votado'}
                  >
                    <ThumbsUp size={14} />
                  </button>
                  
                  {/* Pulgar hacia abajo - Rojo si rechazado, gris si no - INTERACTIVO */}
                  <button
                    ...
                    title={!hasVoted && canVote ? 'Votar Rechazar' : 'Ya votado'}
                  >
                    <ThumbsDown size={14} />
                  </button>
      ...
      {votingStatus === 'voting' && (
        <div className="mt-4 p-3 bg-[#2a2a2a] border border-[#444] rounded-lg text-center">
      ...
      <div className="mt-6 p-3 bg-[#0f0f0f] border border-[#333] rounded-lg">
        <div className="text-xs text-gray-400 text-center">
          <p>• Solo los Darth (Nivel 5) y Maestros (Nivel 6) pueden votar</p>
          <p>• Se requiere unanimidad para aprobar o rechazar una propuesta</p>
