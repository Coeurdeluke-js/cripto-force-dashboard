...
export default function SidebarMaestro({ isCollapsed = false }: SidebarMaestroProps) {
  ...
  useEffect(() => {
    if (userData && !loading) {
      ...
      const currentDashboardLevel = 6; // Maestro = nivel 6
      
      // Mostrar compass solo si el usuario es Fundador (0)
      setShowCompass(userLevel === 0);
      
      console.log('🔍 Sidebar Maestro - Debug:', {
        userLevel,
  ...
  return (
    <div className={`bg-gray-900 text-white transition-all duration-300 ${
      ...
      {/* Compass Icon for Fundador Only */}
      {showCompass && (
